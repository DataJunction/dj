...
export function NamespacePage() {
  ...
  const nodesList = state.nodes.map((node) =>
        <tr>
          ...
          <td>
            <a href={ "/nodes/" + node.name } className="link-table">{node.display_name}</a>
            <span className="rounded-pill badge bg-secondary-soft" style={{marginLeft: "0.5rem"}}>{ node.version }</span>
          </td>
          <td>
            <span className={"node_type__" + node.type + " badge node_type"}>{node.type}</span>
          </td>
          <td>
