...
export function SelectionPanel({
  ...
  const loadedCube = useMemo(() => {
    ...
    return cubes.find(c => c.name === loadedCubeName) || { name: loadedCubeName };
  ...
  const filteredCubes = useMemo(() => {
    ...
    return cubes.filter(cube => {
      ...
      return name.toLowerCase().includes(search) || displayName.toLowerCase().includes(search);
    });
  }, [cubes, cubeSearch]);
          ...
          <div className="preset-row">
            ...
            >
              ...
              <span className="preset-label">
                {loadedCube
                  ? (loadedCube.display_name || (loadedCube.name ? loadedCube.name.split('.').pop() : 'Cube'))
                  : 'Load from Cube'}
              </span>
              <span className="dropdown-arrow">{showCubeDropdown ? '▲' : '▼'}</span>
            </button>
              ...
              <div className="cube-list">
                {filteredCubes.length === 0 ? (
                  <div className="cube-empty">
                    {cubeSearch ? 'No cubes match your search' : 'No cubes available'}
                  </div>
                ) : (
                  filteredCubes.map(cube => (
                    ...
                    >
                      <span className="cube-name">
                        {cube.display_name || (cube.name ? cube.name.split('.').pop() : 'Unknown')}
                      </span>
      ...
      <div className="selection-section">
        ...
        {selectedMetrics.length > 0 && (
          <div className="selected-chips-container">
            <div className={`selected-chips-wrapper ${metricsChipsExpanded ? 'expanded' : ''}`}>
            ...
            {selectedMetrics.length > CHIPS_COLLAPSE_THRESHOLD && (
              ...
              >
                <span>{metricsChipsExpanded ? 'Show less' : `Show all ${selectedMetrics.length}`}</span>
                <span className="chips-toggle-icon">{metricsChipsExpanded ? '▲' : '▼'}</span>
              </button>
            )}
          </div>
        )}
        ...
        {selectedMetrics.length === 0 ? (
          ...
          <>
            ...
            {selectedDimensions.length > 0 && (
              <div className="selected-chips-container">
                <div className={`selected-chips-wrapper ${dimensionsChipsExpanded ? 'expanded' : ''}`}>
                  <div className="selected-chips">
                    {selectedDimensions.map(dimName => (
                      <span key={dimName} className="selected-chip dimension-chip">
                        <span className="chip-label">{getDimDisplayName(dimName)}</span>
                        <button
                          className="chip-remove"
                ...
                {selectedDimensions.length > CHIPS_COLLAPSE_THRESHOLD && (
                  ...
                  >
                    <span>{dimensionsChipsExpanded ? 'Show less' : `Show all ${selectedDimensions.length}`}</span>
                    <span className="chips-toggle-icon">{dimensionsChipsExpanded ? '▲' : '▼'}</span>
                  </button>
                )}
              </div>
            )}
