...
import { PreAggDetailsPanel, MetricDetailsPanel } from './PreAggDetailsPanel';
import './styles.css';
...
export function MaterializationPlannerPage() {
  ...
  // Fetch V3 measures when selection changes
  useEffect(() => {
    const fetchData = async () => {
      if (selectedMetrics.length > 0 && selectedDimensions.length > 0) {
        ...
        try {
          const measures = await djClient.measuresV3(selectedMetrics, selectedDimensions);
