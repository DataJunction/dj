...
from djqs.api.queries import dispatch_query
from djqs.config import Settings
from djqs.constants import DJ_DATABASE_ID
from djqs.engine import process_query
from djqs.errors import DJError, DJException, DJWarning, ErrorCode
...
def test_submit_query_native(mocker: MockerFixture, client: TestClient) -> None:
    ...
    """
    get_query_for_sql = mocker.patch("djqs.api.queries.get_query_for_sql")
    mocker.patch(
...
def test_submit_query_native_error(mocker: MockerFixture, client: TestClient) -> None:
    ...
    Test ``POST /queries/`` with a native DJ query.
    """
    mocker.patch("djqs.api.queries.get_query_for_sql")
    mocker.patch(
        "djqs.api.queries.save_query_and_run",
...
def test_submit_query_async_celery(
    ...
    dispatch_query = mocker.patch(  # pylint: disable=redefined-outer-name
        "djqs.api.queries.dispatch_query",
    )
...
def test_dispatch_query(
    ...
    Test ``dispatch_query``.
    """
    get_session = mocker.patch("djqs.api.queries.get_session")
    get_session().__next__.return_value = (  # pylint: disable=redefined-outer-name
        session
    )

    mocker.patch("djqs.api.queries.get_settings", return_value=settings)
...
def test_pagination(
    ...
    mocker.patch("djqs.config.RedisCache", return_value=cache)
