...
@pytest.mark.asyncio
async def test_visualize_metrics_no_materialized_cube(mock_plotext):
    """Test visualize_metrics when no materialized cube is available"""
    ...
    with (
        ...
        assert len(result) == 1
        assert "No materialized cube available" in result[0].text
        assert "Visualization requires a materialized cube" in result[0].text
        assert "test.metric" in result[0].text
