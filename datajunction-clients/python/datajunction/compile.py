...
class Deployment:
    ...
    def clean_dict(d: dict) -> dict:
        ...
        for k, v in d.items():
            ...
            if isinstance(v, dict):
                nested = Deployment.clean_dict(v)
    ...
    def pull(
        ...
        if any(path.iterdir()) and not ignore_existing_files:
            raise DJClientException("The target path must be empty")
        deployment_spec = client._export_namespace_spec(namespace)
        ...
        for node in nodes:
            ...
            with open(file_path, "w") as yaml_file:
                yaml.dump(Deployment.clean_dict(node), yaml_file, sort_keys=False)
    ...
    def push(
        ...
        deployment_spec = Deployment.reconstruct_deployment_spec(source_path)
        deployment_data = client.deploy(deployment_spec)
        ...
        with Live(
            Deployment.build_table(deployment_uuid, deployment_data),
            console=console,
            screen=False,
            ...
            while deployment_data.get("status") not in ("failed", "success"):
                time.sleep(1)
                deployment_data = client.check_deployment(deployment_uuid)
                live.update(Deployment.build_table(deployment_uuid, deployment_data))
    ...
    @staticmethod
    def collect_nodes_from_dir(base_dir: str | Path) -> List[Dict[str, Any]]:
        """
        ...
        for path in nodes_dir.rglob("*.yaml"):
            if path.name == "dj.yaml":
                continue
            node_dict = Deployment.read_yaml_file(path)
            nodes.append(node_dict)
        return nodes

    @staticmethod
    def read_project_yaml(base_dir: str | Path) -> Dict[str, Any]:
        """
        Reads project-level dj.yaml
        """
        project_path = Path(base_dir) / "dj.yaml"
        if project_path.exists():
            return Deployment.read_yaml_file(project_path)
        return {}

    @staticmethod
    def reconstruct_deployment_spec(base_dir: str | Path) -> Dict[str, Any]:
        """
        Reads exported YAML files and reconstructs a DeploymentSpec-compatible dict.
        """
        project_metadata = Deployment.read_project_yaml(base_dir)
        nodes = Deployment.collect_nodes_from_dir(base_dir)
        deployment_spec = {
