---
name: datajunction-build
description: |
  Activate this skill when creating, authoring, or modifying DataJunction (DJ)
  nodes — metrics, dimensions, transforms, cubes — via the API or repo-backed
  YAML. Includes the query-to-DJ decomposition workflow for turning a draft
  SQL query into a properly modeled set of nodes. For core DJ concepts and
  vocabulary, invoke `datajunction`. For querying existing metrics, invoke
  `datajunction-query`.
  Keywords:
...
Author-side workflow for DJ: create metrics/dimensions/transforms/cubes, decompose draft queries into well-modeled nodes, manage a repo-backed namespace via YAML and git.

For DJ concepts and vocabulary (node types, dimension links, star schema, mode/status), see the `datajunction` skill — this skill assumes you're familiar.
...
**Best approach - Use MCP tool `get_node_details`** (see `datajunction-query` skill for full tool reference):
...
parent table or dimension, use the `datajunction-query` skill's MCP tools
(`search_nodes`, `get_node_details`) to check whether DJ already has an
authoritative node for it. Prefer building on existing nodes — every
additional transform fragments the catalog. When a referenced table isn't
in any git-backed namespace, flag it: the user may want to author the
parent first (or reconsider the source) rather than create a sibling node.
...
join cost even when nobody's slicing by that dim. See the `datajunction`
skill's "Dimension Links" section for the mechanics.
...
**Critical**: All metrics in a cube MUST share ALL dimensions in the cube. Use `get_common_dimensions` MCP tool (see `datajunction-query` skill) to check first!
...
Use MCP tool (see `datajunction-query` skill):
```
get_node_details(name="finance.total_revenue")
...
- ✅ Use `get_common_dimensions` MCP tool (`datajunction-query` skill) to check compatibility first
