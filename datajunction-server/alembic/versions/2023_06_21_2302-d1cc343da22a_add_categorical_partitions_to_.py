...
def upgrade():
    with batch_alter_table("availabilitystate") as batch_op:
        batch_op.add_column(sa.Column('categorical_partitions', sa.JSON(), nullable=True))
        batch_op.add_column(sa.Column('temporal_partitions', sa.JSON(), nullable=True))
        batch_op.add_column(sa.Column('min_temporal_partition', sa.JSON(), nullable=True))
        batch_op.add_column(sa.Column('max_temporal_partition', sa.JSON(), nullable=True))
...
def downgrade():
    with batch_alter_table("availabilitystate") as batch_op:
        batch_op.add_column(sa.Column('max_partition', sqlite.JSON(), nullable=True))
        batch_op.add_column(sa.Column('min_partition', sqlite.JSON(), nullable=True))
