...
from datajunction_server.models.user import User, OAuthProvider
...
def test_dj_http_bearer_w_cookie():
    ...
    asyncio.run(bearer(request))
    assert request.state.user == User(id=1, username='dj', password=None, email=None, name=None, oauth_provider=OAuthProvider.BASIC, is_admin=False)
