 ...
 SELECT  default_DOT_repair_orders_fact_DOT_repair_order_id,
  ...
  FROM combiner_query
  WHERE  default_DOT_repair_orders_fact_DOT_order_date = CAST(DATE_FORMAT(CAST(${dj_logical_timestamp} AS TIMESTAMP), 'yyyyMMdd') AS TIMESTAMP) AND default_DOT_dispatcher_DOT_company_name = CAST(DJ_COMPANY_NAME() AS STRING)
