...
class TestBranchManagement:
    ...
    async def test_create_branch_without_git_config(
        ...
        assert "does not have git configured" in response.json()["message"]
    ...
    async def test_create_branch_without_git_branch(
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST
        assert "does not have a git branch configured" in response.json()["message"]
    ...
    async def test_create_branch_github_error(
        ...
        with patch(
            ...
            assert response.status_code == HTTPStatus.BAD_REQUEST
            assert "Failed to create git branch" in response.json()["message"]
    ...
    async def test_delete_branch_not_a_child(
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST
        assert "is not a branch of" in response.json()["message"]
...
class TestGitSync:
    ...
    async def test_sync_namespace_empty(
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST
        assert "has no nodes to sync" in response.json()["message"]
...
class TestPullRequest:
    ...
    async def test_create_pr_not_branch_namespace(
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST
        assert "is not a branch namespace" in response.json()["message"]
...
class TestNodeFilePath:
    """Tests for the _get_node_file_path helper function."""

    def test_file_path_simple(self):
        """Test file path for simple node name."""
        from datajunction_server.api.git_sync import _get_node_file_path

        path = _get_node_file_path("myns.my_node", "myns", None)
        assert path == "my_node.yaml"

    def test_file_path_nested(self):
        """Test file path for nested node name."""
        from datajunction_server.api.git_sync import _get_node_file_path

        path = _get_node_file_path("myns.sub.deep.my_node", "myns", None)
        assert path == "sub/deep/my_node.yaml"

    def test_file_path_with_git_path(self):
        """Test file path with git_path prefix."""
        from datajunction_server.api.git_sync import _get_node_file_path

        path = _get_node_file_path("myns.my_node", "myns", "definitions/nodes")
        assert path == "definitions/nodes/my_node.yaml"

    def test_file_path_with_leading_trailing_slashes(self):
        """Test file path strips leading/trailing slashes from git_path."""
        from datajunction_server.api.git_sync import _get_node_file_path
