...
async def test_measures_sql_agg_distinct_metric(
    ...
    assert metric_data["measures"] == [
        {
            "aggregation": None,
            "expression": "DISTINCT hard_hat_id",
            "name": "hard_hat_id_distinct_2673ee49",
            "rule": {"level": ["hard_hat_id"], "type": "limited"},
        },
    ]
    assert (
        metric_data["derived_expression"]
        == "COUNT( DISTINCT hard_hat_id_distinct_2673ee49)"
    )
    ...
    SELECT
      ...
      hard_hat_id AS hard_hat_id_distinct_2673ee49
    FROM default_DOT_repair_orders_fact_built
