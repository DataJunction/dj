...
async def test_create_node_auto_creates_owner_role(
    ...
    """
    ns_name = unique_node_namespace.make("nodeautorole")
    node_name = f"{ns_name}.test_source"
    unique_node_namespace.track_node(node_name)
    ...
    response = await client.get(f"/roles/{node_name}-owner")
    assert response.status_code == 200
    role_data = response.json()
    assert role_data["name"] == f"{node_name}-owner"
    assert role_data["description"] == f"Owner role for node {node_name}"
    ...
    assert scope["scope_value"] == node_name

    # Check the role is assigned to the creator
    response = await client.get(f"/roles/{node_name}-owner/assignments")
    assert response.status_code == 200
...
async def test_create_transform_node_auto_creates_owner_role(
    ...
    """
    ns_name = unique_node_namespace.make("transformrole")
    source_name = f"{ns_name}.source"
    transform_name = f"{ns_name}.my_transform"
    unique_node_namespace.track_node(source_name)
    unique_node_namespace.track_node(transform_name)

    # First create namespace and source node
    await client.post(f"/namespaces/{ns_name}")
    ...
    response = await client.post(
        ...
        json={
            ...
            "query": f"SELECT id, value FROM {source_name}",
            "mode": "published",
        },
    )
    assert response.status_code in (200, 201)
    ...
    response = await client.get(f"/roles/{transform_name}-owner")
    assert response.status_code == 200
    role_data = response.json()
    assert role_data["name"] == f"{transform_name}-owner"
    assert len(role_data["scopes"]) == 1
