...
class TestErrorHandling:
    ...
    async def test_circular_metric_dependency_detection(self, client_with_build_v3):
        """
        Test that circular dependencies between derived metrics are detected.

        Creates two derived metrics that reference each other:
        - metric_a = metric_b + 1
        - metric_b = metric_a + 1

        This should raise an error when we try to generate SQL.
        """
        # Create first circular metric: references metric_b
        response = await client_with_build_v3.post(
            "/nodes/metric/",
            json={
                "name": "v3.circular_metric_a",
                "display_name": "Circular Metric A",
                "description": "A metric that references circular_metric_b",
                "query": "SELECT v3.circular_metric_b + 1 AS v3_circular_metric_a",
                "mode": "published",
            },
        )
        # May fail if metric_b doesn't exist yet, that's OK - continue

        # Create second circular metric: references metric_a
        response = await client_with_build_v3.post(
            "/nodes/metric/",
            json={
                "name": "v3.circular_metric_b",
                "display_name": "Circular Metric B",
                "description": "A metric that references circular_metric_a",
                "query": "SELECT v3.circular_metric_a + 1 AS v3_circular_metric_b",
                "mode": "published",
            },
        )
        # May also fail, that's expected with circular deps

        # Try to generate SQL for circular metrics - should fail
        response = await client_with_build_v3.get(
            "/sql/metrics/v3/",
            params={
                "metrics": ["v3.circular_metric_a"],
                "dimensions": [],
            },
        )

        # Should either fail with circular dependency error or fail to create
        # The key is that we don't hang forever or crash
        ...
        # If it returns an error, check the message mentions the issue
