...
BUILD_V3 = (  # type: ignore
    ...
    (
        "/nodes/metric/",
        {
            ...
            "query": """
                SELECT
                    (CAST(v3.order_count AS DOUBLE) - LAG(CAST(v3.order_count AS DOUBLE), 1) OVER (ORDER BY v3.date.week))
                    / NULLIF(LAG(CAST(v3.order_count AS DOUBLE), 1) OVER (ORDER BY v3.date.week), 0) * 100
            """,
            "mode": "published",
        },
    ),
