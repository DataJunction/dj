...
async def get_nodes_with_common_dimensions(
    ...
    Uses the pre-computed dimension_reachability table which stores (node_id, dimension_id)
    pairs for ALL nodes, enabling O(1) lookups instead of expensive graph traversals.

    Args:
        common_dimensions: List of dimension nodes to find common nodes for
        node_types: Optional list of node types to filter by
        options: SQLAlchemy query options for eager loading (ignored if names_only=True)
        names_only: If True, returns just node names (faster). If False, returns NodeRevision objects.

    Returns:
        List of NodeRevision objects, or list of node name strings if names_only=True
    """
    ...
    # Direct lookup from the reachability table:
    # Find nodes that can reach ALL target dimensions
    nodes_with_all_dims = (
        select(DimensionReachability.source_node_id.label("node_id"))
        .where(DimensionReachability.target_dimension_node_id.in_(dimension_ids))
        .group_by(DimensionReachability.source_node_id)
        .having(
    ...
    statement = (
        ...
        .join(
            nodes_with_all_dims,
            Node.id == nodes_with_all_dims.c.node_id,
        )
