...
def _diff_col_maps(
    ...
    """Return ``(removed_names, changed_triples)`` by comparing two column lists.

    Both lists must contain objects with ``.name`` and ``.type`` attributes
    (ORM ``Column``, ``ColumnSpec``, or ``_ProposedColumn`` all work).
    ``changed_triples`` are ``(name, old_type_str, new_type_str)`` tuples.
    """
    ...
    changed = [
        (name, str(old_map[name].type), str(new_map[name].type))
        for name in old_map
        if name in new_map and str(old_map[name].type) != str(new_map[name].type)
    ]
...
async def _check_immediate_metric_cube_impacts(
    ...
    for metric in immediate_metrics:
        ...
        for cube in cubes:
            if cube.type != NodeType.CUBE or cube.name in seen_cubes:
            ...
            for element in cube.current.cube_elements:  # pragma: no branch
                source_node_name = (
                ...
                if (
                    source_node_name and source_node_name in dim_links_removed
                ):  # pragma: no branch
                    _record_impact(
                        impacted,
                        cube,
...
class _ExplicitDiffSpec:
    ...
    When ``explicit_proposed_columns`` is set (deployment path), ``_compute_seed_prop``
    uses it directly and skips all lazy-loaded ``existing_node.current.*`` access.
    This is required when the session has already committed/rolled back and ORM objects
    are expired.
    """
    ...
    dim_links_removed: set[str] = field(default_factory=set)
    explicit_proposed_columns: list | None = None  # list[_ProposedColumn], pre-computed
...
async def _infer_column_diff(
    ...
    """Parse ``new_query`` through the bulk validator to derive the actual column diff.
    ...
    spec = spec.model_copy(update={"query": new_query})
    ...
    if result.status != NodeStatus.VALID or not result.inferred_columns:
        errors = [e.message for e in (result.errors or [])]
        return [], [], [], False, errors

    columns_removed, columns_changed = _diff_col_maps(
        node.current.columns,
        result.inferred_columns,
    )
    proposed_columns = [
        _ProposedColumn(name=c.name, type=c.type) for c in result.inferred_columns
    ]
...
async def _validate_downstream_node(
    ...
    try:
        spec = await child.to_spec(session)
        # Clear pre-existing columns so the bulk validator infers types from the SQL
        # AST instead of using cached column metadata.  Without this, _create_column_spec
        ...
        _logger.warning(
            "exception validating downstream node %r: %r",
            child.name,
            exc,
        )
        return True, []

    if result.status != NodeStatus.VALID or result.errors:
        return True, []

    return False, list(result.inferred_columns) if result.inferred_columns else list(
...
async def _batch_get_children(
    ...
    for row in rows:
        if row.child_id in child_by_id:  # pragma: no branch
...
async def _compute_seed_prop(
    ...
    if isinstance(proposed_spec, _ExplicitDiffSpec):
        ...
        if proposed_spec.explicit_proposed_columns is not None:
            # Deployment path: everything pre-computed before session commit/rollback.
            # dim_links_removed already includes FK-on-removed-columns links.
            prop.proposed_columns = proposed_spec.explicit_proposed_columns
            prop.columns_removed = proposed_spec.explicit_columns_removed
            prop.columns_changed = list(proposed_spec.explicit_columns_changed)
            # Empty proposed columns means the new query is broken → node is invalid.
            # Mark is_invalid so the BFS propagates unconditionally to cubes and
            # other children that have no SQL query to validate against.
            if not prop.proposed_columns:
                prop.is_invalid = True
            return prop  # Skip existing_node.current.* access below
        elif proposed_spec.new_query:
    ...
    # Empty proposed columns means the new query is broken → mark invalid so the
    # BFS propagates unconditionally to cubes and other no-SQL-query children.
    if prop.proposed_columns is not None and not prop.proposed_columns:
        prop.is_invalid = True
...
async def _seed_bfs(
    ...
    for name, prop in state.propagated_change.items():
        if prop.dim_links_removed:
            ...
            if name in state.node_cache:  # pragma: no branch
...
async def _propagate_column_change(
    ...
    if prop.is_deleted or prop.is_invalid:
        ...
        if child_name not in state.visited:  # pragma: no branch
...
async def compute_impact(
    ...
    session:
        Async DB session.
    changed_nodes:
        Mapping of node_name to (proposed_spec, existing_node).
