...
async def propagate_impact(
...
) -> tuple[list[DownstreamImpact], dict[str, tuple[str, ast.Query]]]:
    ...
    Returns:
        DownstreamImpact list, plus the ASTs this already parsed while
        revalidating (source/transform/dimension queries only — metrics
        parse an alias-wrapped string here, so their AST isn't reusable
        for fingerprinting). Reused by the fingerprint pass to skip
        re-parsing the same query text.
    """
    ...
    results, pre_parsed_queries = await _revalidate_and_apply(session, ctx, all_impacts)

    _emit_metrics(start, results)
    return results, pre_parsed_queries
...
async def _revalidate_and_apply(
    ...
    # Reusable by fingerprinting: non-metric queries only, paired with the
    # exact text parsed so the caller can verify it still matches before
    # trusting the AST. Metrics are parsed here in raw form, but
    # fingerprinting hashes an alias-wrapped parse of the same query, so a
    # metric's AST here isn't a substitute.
    reusable_asts = {
        name: (queries_to_parse[name], parsed)
