...
async def get_a_tag(name: str, *, session: AsyncSession = Depends(get_session)) -> TagOutput:
    """
...
async def update_a_tag(
    ...
    Update a tag.
    """
    tag = await get_tag_by_name(session, name, raise_if_not_exists=True, for_update=True)
