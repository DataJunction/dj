    ...
    return (
        TranslatedSQL(
            ...
            upstream_tables=[
                ...
                # If a table has a corresponding node type,
                # then it must be either a Source node or some type of Materialized node.
                # Even if there is no availability state right now, we this qery may be built
                # for future execution, so we don't need to check for availability state here.
                if tbl.dj_node and tbl.dj_node.type
            ],
        ),
        engine,
