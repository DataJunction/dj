...
@router.post("/nodes/{name}/savecolumnorder")
def save_column_order(
    ...
    _logger.info("Processing %s node `%s`", node.type, node.name)

    # Only assign column orders if one or more columns lack an order value
    columns_without_order = [col.order for col in current_node_revision.columns if not col.order]
    if columns_without_order:
        _logger.info("Found columns without order for %s node `%s`", node.type, node.name)
        if node.type == NodeType.SOURCE:
            column_mapping = {
                col.name: col
                for col in query_service_client.get_columns_for_table(
                    current_node_revision.catalog.name,
                    current_node_revision.schema_,
                    current_node_revision.table,
                )
            }
            for col in current_node_revision.columns:
                col.order = column_mapping[col.name].order
                session.add(col)
            _logger.info("Assigning new column order to %s node `%s`", node.type, node.name)
            session.commit()
            return JSONResponse(
                status_code=200,
                content={
                    "message": f"Assigned new column order to node `{node.name}`",
                    "columns": [ColumnOutput.from_orm(col) for col in current_node_revision.columns],
                },
            )
        if node.type in (NodeType.TRANSFORM, NodeType.DIMENSION, NodeType.METRIC):
            query_ast = parse(current_node_revision.query)
            column_mapping = {
                col.alias_or_name.name: idx
                for idx, col in enumerate(query_ast.select.projection)
            }
            for col in current_node_revision.columns:
                col.order = column_mapping[col.name]
                session.add(col)
            _logger.info("Assigning new column order to %s node `%s`", node.type, node.name)
            session.commit()
            ...
            return JSONResponse(
                ...
                content={
                    "message": f"Assigned new column order to node `{node.name}`",
                    "columns": [
                        ColumnOutput.from_orm(col).dict()
                        for col in current_node_revision.columns
                    ],
                },
            )
        ...
        return JSONResponse(
            status_code=500,
            content={
                "message": f"Manual update needed for `{node.name}`.",
                "columns": [ColumnOutput.from_orm(col) for col in current_node_revision.columns],
            },
        )
    return JSONResponse(
        status_code=200,
        content={
            "message": f"All columns for `{node.name}` have an order. Skipping update."
        }
    )
