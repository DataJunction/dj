...
async def list_catalogs(*, session: AsyncSession = Depends(get_session)) -> List[CatalogInfo]:
    """
    List all available catalogs
    """
    return [
        CatalogInfo.from_orm(catalog)
        for catalog in (await session.execute(select(Catalog))).scalars()
    ]


@router.get("/catalogs/{name}/", response_model=CatalogInfo, name="Get a Catalog")
async def get_catalog(name: str, *, session: AsyncSession = Depends(get_session)) -> CatalogInfo:
    """
