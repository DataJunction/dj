...
class PydanticListType(TypeDecorator):
    ...
    def process_bind_param(
        self, value: Optional[List[T]], dialect
    ) -> Optional[List[Dict]]:
    ...
    def process_result_value(
        self, value: Optional[List[Dict]], dialect
    ) -> Optional[List[T]]:
...
def compute_preagg_slug(node_name: str, preagg_hash: str) -> str:
    """
    Compute the slug for a pre-aggregation.

    Format: {node_name}-{hash}
    Example: common.orders.orders_fact-abc12345

    The slug is:
    - Human-readable: Node name is searchable and meaningful
    - Unique: Hash disambiguates different grain/measure combinations
    - URL-safe: Uses '-' separator which is safe in URLs

    Args:
        node_name: Full node name (e.g., "common.orders.orders_fact")
        preagg_hash: The 8-char hash from compute_preagg_hash()

    Returns:
        Slug string for API lookups
    """
    return f"{node_name}-{preagg_hash}"
...
class PreAggregation(Base):
    ...
    columns: Mapped[Optional[List[V3ColumnMetadata]]] = mapped_column(
        ...
        nullable=True,  # Nullable for backward compatibility with existing pre-aggs
    )
    ...
    # Slug: human-readable unique identifier for API lookups
    # Format: {node_name}-{hash} e.g., "common.orders.orders_fact-abc12345"
    # The hash includes node_revision_id + grain + measures for uniqueness
