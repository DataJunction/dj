...
def get_measure_expr_hashes(measures: List[Dict[str, Any]]) -> Set[str]:
    """
    Extract expression hashes from a list of measures.

    Args:
        measures: List of measure dicts with 'expr_hash' field

    Returns:
        Set of expression hashes
    """
    return {m.get("expr_hash", "") for m in measures if m.get("expr_hash")}
...
class PreAggregation(Base):
    ...
    # Each measure is a dict with: name, expression, expr_hash, aggregation, merge, rule
    measures: Mapped[List[Dict[str, Any]]] = mapped_column(JSON, nullable=False)
