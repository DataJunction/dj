...
class QueryServiceClient:  # pylint: disable=too-few-public-methods
    ...
    def get_columns_for_table(
        ...
        """
        response = self.requests_session.get(f"/table/{catalog}.{schema}.{table}/columns/")
    ...
    def get_available_engines(
        ...
        """
        Retrieves engines configured in the query service for a catalog.
        """
        response = self.requests_session.get(
            f"/catalogs/{catalog}/",
        )
        available_engines = response.json()["engines"]
        return [
            EngineInfo(name=engine["name"], version=engine["version"])
